...
SDMVSTRA / SDMVPSU when available, otherwise weighted SE (noted in the output).
...
def wse(x, wt):
    """Weighted SE; Taylor-linearized by stratum/PSU when design vars exist."""
    m = np.isfinite(x) & np.isfinite(wt)
    x, wt = x[m], wt[m]
    if not len(x):
        return np.nan
    mu = np.average(x, weights=wt)
    v = np.sum(wt ** 2 * (x - mu) ** 2) / (np.sum(wt) ** 2)
    return np.sqrt(v)
...
def add_cont(label, col, dp=1):
    ...
    for name, sub in [("Overall", np.ones(len(an), bool)),
        ...
        r[name] = f"{wmean(x[sub], w[sub]):.{dp}f} ({wse(x[sub], w[sub]):.{dp+1}f})"
    rows.append(r)
...
hdr = (f"Table 1. Baseline characteristics of the analytic cohort "
       ...
       f"Weighted estimates use WTMEC2YR. Continuous variables: weighted mean (SE). "
       f"Categorical: weighted %.\n"
