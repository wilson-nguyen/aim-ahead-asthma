...
for tracked files are computed from the committed blob bytes at the named
commit; verify with `git show <commit>:<path> | sha256sum` (blob content),
not by hashing working-tree files, whose bytes differ under CRLF checkout.
The manifest documents the state at that commit and is itself
